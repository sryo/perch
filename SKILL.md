...
| `screenshot` | PNG of the target browser window. Captures by CGWindowID so obscured windows work without focus steal. If `tabIndex` targets a non-active tab, perch silently switches the window to it first (Chrome/Safari; Arc no-ops). `raise: true` brings the window forward. |
