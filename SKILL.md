...
| `screenshot` | PNG of the target browser window. Captures by CGWindowID so obscured windows work without focus steal. Only the active tab in a window is rendered, so `tabIndex` doesn't switch tabs; `activate_tab` first if you need a specific tab. `raise: true` brings the window forward. |
